...
from yolo3.model_Mobilenet import yolo_eval, yolo_body, tiny_yolo_body, mobilenetv2_yolo_body
...
class YOLO(object):
    ...
    def detect_image(self, image, det_area=(0, 0, 0.9, 1), show_bb=True, show_det_area=True):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            bbox_overlap = False
            bbox_size = False
            x_overlap = max(0, min(right, det_area_right) - max(left, det_area_left))
            y_overlap =max(0, min(bottom, det_area_bot) - max(top, det_area_top))
            overlap_area = x_overlap * y_overlap
            bbox_area = abs(right - left) * abs(bottom - top)
            
            if overlap_area > 0.75 * bbox_area:
                bbox_overlap = True
